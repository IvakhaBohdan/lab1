# Розрахунково-графічна робота та Лабораторна робота №2: Консольний додаток для СУБД PostgreSQL

**Виконавець:** Іваха Богдан група КВ-32
...
Цей проєкт є консольним додатком на **Python**, розробленим за архітектурним шаблоном **MVC (Model-View-Controller)**.

### Еволюція та Мета Проєкту
Спочатку додаток реалізовував CRUD-операції та складні запити за допомогою чистого SQL. Після виконання **Лабораторної роботи №2**, модуль **Model** був перетворений на **Об'єктно-Реляційну Проекцію (ORM)**, що є ключовою частиною вимог до фінального проєкту.

Проєкт охоплює:
1. **Функціонал CRUD** та складні пошукові запити.
2. **Оптимізацію БД** (Індекси, Тригери).
3. **Аналіз Транзакцій** (Рівні ізоляції).

### Архітектура MVC
* **Model (Оновлено)**: Відповідає за всю логіку взаємодії з БД через **SQLAlchemy ORM** (класи-сутності та зв'язки).
...
## 3. Технології та Вимоги Лабораторної роботи №2

* **Мова програмування:** Python 3.11
* **Система управління базами даних (СУБД):** PostgreSQL
* **Інструмент БД:** PgAdmin4
* **Бібліотеки Python:**
    * **SQLAlchemy** (використовується для ORM, замість Psycopg 2 у модулі Model)
    * **Psycopg 2** (може використовуватися для низькорівневих операцій)
